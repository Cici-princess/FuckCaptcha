CNN_KERAS_DIR = r'F:\MyProjects\PycharmProjects\FuckCaptcha\CNN_Keras'


import os
os.chdir(CNN_KERAS_DIR)
import numpy as np
import argparse
import cv2
...
dataset = fetch_openml('MNIST Original',data_home=CNN_KERAS_DIR)
#要把这个matlab文件放在上述地址的mldata文件夹下  dataset COL_NAMES ['label','data_info']   DESCR str   data_info 70000*784  target 70000*1

# Read the MNIST data_info as array of 784 pixels and convert to 28x28 image matrix
mnist_data = dataset.data.reshape((dataset.data.shape[0], 28, 28))
mnist_data = mnist_data[:, np.newaxis, :, :]

# Divide data_info into testing and training sets.
...
# Transform training and testing data_info to 10 classes in range [0,classes] ; num. of classes = 0 to 9 = 10 classes
...
# Initially train and run_testing the model; If weight saved already, load the weights using arguments.
b_size = 128	# Batch size 128	
...
if args["load_model"] < 0:
	...
	# Evaluate accuracy and loss function of run_testing data_info
