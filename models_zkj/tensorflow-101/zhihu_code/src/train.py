...
tf.app.flags.DEFINE_string('test_data_dir','../data/test_data','the run_testing dataset dir')
        ...
        try:
            while not coord.should_stop():
                ...
                if step % FLAGS.eval_steps == 1:
                    ...
                    # can't run 
                    test_images_batch, test_labels_batch = sess.run([test_images, test_labels])
                    logger.info('[run_testing] gen run_testing batch spend {0}'.format(time.time()-start_time))
                    feed_dict = {
                    ...
                    logger.info( '[run_testing] the step {0} accuracy {1} spend time {2}'.format(step, accuracy_val, (end_time-start_time)))
